...
request omits ``thinking``.  Cortes' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Cortes reasoning
...
(per Z.AI / BigModel docs).  Cortes' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Cortes reasoning effort onto GLM-5.2's native ``high``/``max``.
