"""
Photon Spectrum (iMessage) platform adapter for Cortes Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Cortes dep
...
# artifacts so Cortes sees the link once, not a follow-up "(attachment)" prompt.
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Cortes does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Cortes defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Cortes-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Cortes checkout's sidecar entry point.
...
async def _standalone_send(
    ...
    *,
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Cortes gateway (which spawns the sidecar and "
    ...
    headers = {"X-Cortes-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Cortes plugin loader at startup."""
