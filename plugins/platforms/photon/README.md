...
This plugin connects Cortes Agent to iMessage (and other Spectrum
...
TypeScript-only, Cortes runs it inside a small supervised Node sidecar and
...
2. **Find or create** the `Cortes Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Cortes channel,
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Cortes
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Cortes coalesces
...
- **Native polls are supported.** Cortes posts poll content through
