...
Small Node helper that bridges Cortes Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Cortes is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Cortes-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Cortes can receive inbound messages
...
The Cortes plugin's `hermes photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Cortes is Python, so replies go through this sidecar
