...
and injects content-free wake hints into Cortes' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Cortes only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Cortes turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    ...
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Cortes env
    ...
    print_info("Connect Cortes to Raft as an external agent.")
