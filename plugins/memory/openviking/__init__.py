...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Cortes peer ID in OpenViking (default: hermes)
...
_AGENT_PROMPT_LABEL = "Cortes peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Cortes slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Cortes to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Cortes refused the connection."
        ) from exc
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Cortes did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Cortes will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Cortes session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Cortes only", "write values only to Cortes .env"),
    ...
    _print_openviking_ready("Connection saved to Cortes .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Cortes peer ID in OpenViking, sent as the actor peer and "
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Cortes will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Cortes will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Cortes' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Cortes canonical messages into OpenViking batch payloads."""
