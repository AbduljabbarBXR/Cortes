...
`https://api.supermemory.ai`. Cortes resolves it once and uses the same endpoint
...
it groups Cortes-written memories into a dedicated "Cortes" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Cortes profile:
