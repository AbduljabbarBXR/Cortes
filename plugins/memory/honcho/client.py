...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Cortes instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Cortes profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Cortes profile.
    ...
    # but applying it before profile resolution makes every named Cortes
    # profile share that same host.  Keep named profiles isolated; only the
    # default Cortes profile may opt into the config's default host.
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Cortes profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # baseUrl / base_url keys below are the Cortes-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Cortes session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Cortes session title (from /title command; non-per-session)
