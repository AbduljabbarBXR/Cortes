...
Optional Cortes observability plugin that configures exporters and maps
Cortes-specific observer hooks to NeMo Relay marks and ATIF state. Cortes core
...
not replace Cortes Agent's planner, tools, memory, model provider routing, or
CLI UX. Cortes core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Cortes Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Cortes core.
- Add Cortes session, turn, approval, and subagent mark events.
...
`HERMES_HOME` is the Cortes profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
Cortes uses the user's default home, usually `~/.hermes`. For isolated smoke
...
wheel, Cortes installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control Cortes shared client metrics.
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Cortes does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Cortes keeps the direct env-var fallbacks active for that run.

Cortes core routes provider and tool execution through NeMo Relay managed APIs
...
Cortes uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Cortes-owned
...
with this Cortes-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Cortes rejects
...
Cortes activates these plugins before registering its managed LLM and tool
...
For the full generic Cortes middleware contract, see
...
with Cortes and a local Ollama model served through the OpenAI-compatible API.
...
This run starts a parent Cortes session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Cortes Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Cortes Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Cortes dispatches the read-only tools as one batch, and NeMo Relay
...
- Cortes sessions map to `agent` scopes.
- Cortes core managed provider calls map to `llm` scope start/end events.
- Cortes core managed tool calls map to `tool` scope start/end events.
...
Cortes core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Cortes-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Cortes calls the
...
```text
Cortes provider call
  -> nemo_relay.llm.execute(...)
    -> Cortes provider adapter callback(...)

Cortes tool call
  -> nemo_relay.tools.execute(...)
    -> Cortes authorization and dispatch callback(...)
```
...
middleware for a local Cortes run. This path requires a NeMo Relay runtime that
...
agent_name = "Cortes Middleware E2E"
agent_version = "local"
