"""nemo_relay — optional Cortes plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atif_agent_name: str = "Cortes Agent"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Cortes profile; keeping the existing process-global configuration "
...
class _Runtime:
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Cortes core Relay session is unavailable")
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Cortes installs the managed execution
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("HERMES_NEMO_RELAY_ATIF_AGENT_NAME") or "Cortes Agent",
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Cortes cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Cortes-owned "
