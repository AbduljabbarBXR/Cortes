"""langfuse — Cortes plugin for Langfuse observability.

Traces Cortes conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Cortes plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Cortes plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Cortes paths only expose ``task_id``/``session_id``. Newer paths
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Cortes usage into Langfuse usage and cost maps."""
    ...
    # Cortes estimate rather than summing components because it also includes
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Cortes branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Cortes also has a turn-scoped
    ...
    # messages is a list only for legacy Cortes branches that fired
    # pre_llm_call with API messages directly. Current Cortes fires
...
def register(ctx) -> None:
    ...
    # Cortes versions.  pre_api_request / post_api_request fire per API
