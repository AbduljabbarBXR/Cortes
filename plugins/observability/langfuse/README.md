...
This plugin ships bundled with Cortes but is **opt-in** — it only loads when
...
hermes chat -q "hello"              # then check Langfuse for a "Cortes turn" trace
```

Generation observations include the Cortes system prompt when the provider
