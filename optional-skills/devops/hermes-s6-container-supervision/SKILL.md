...
description: Modify or debug s6 services in the Cortes Docker image.
version: 1.0.0
author: Cortes Agent
license: MIT
platforms: [linux]
...
# Cortes s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Cortes Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the Cortes Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
- `hermes-tool-quirks`: Specific Cortes-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
