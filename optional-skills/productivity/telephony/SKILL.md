...
This optional skill gives Cortes practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn Cortes into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Cortes memory
...
### 1) "I want Cortes to own a real phone number"
Use **Twilio**.
...
- pairs well with Cortes `text_to_speech` plus a public file host or tunnel
...
Then save credentials into Cortes:
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Cortes calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Cortes's existing `text_to_speech` support.

Use this when:
- you want the call to use Cortes's configured TTS voice rather than Twilio `<Say>`
...
Recommended Cortes TTS -> Twilio Play workflow:

1. Generate the audio with Cortes `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Cortes to create the message audio with `text_to_speech`
...
Important note:
- Cortes TTS is great for prerecorded outbound messages
...
- Cortes STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Cortes memory.
...
- Do not store arbitrary third-party phone numbers in Cortes memory.
