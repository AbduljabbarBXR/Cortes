...
The design is **Cortes-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
Then start a new Cortes session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Cortes session:
...
- **PII handling.** Dossiers live under the Cortes home directory (`0600`, optionally
