...
version: 0.2.0
author: Tony Simons (asimons81), Cortes Agent
license: MIT
...
commands through the Cortes `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from Cortes itself.
