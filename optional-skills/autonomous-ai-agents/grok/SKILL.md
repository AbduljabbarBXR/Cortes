...
version: 0.1.1
author: Matt Maximo (MattMaximo), Cortes Agent
license: MIT
...
# Grok Build CLI — Cortes Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Cortes terminal. Grok
...
1. Prepare stable input files first with Cortes tools (`read_file`,
...
2. **Don't conflate Cortes' xAI auth with the `grok` CLI's auth.** Cortes'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Cortes' xAI auth carries
   over.
