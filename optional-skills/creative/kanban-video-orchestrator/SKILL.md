...
short to a music video to an ASCII loop — in a Cortes Kanban pipeline that
...
3. **Generates** a setup script that creates Cortes profiles, project workspace, and the initial kanban task
...
For mapping role → which Cortes skills + toolsets it loads, see
...
3. Creates each Cortes profile via `hermes profile create --clone`
