...
the actual `setup.sh` that creates the project workspace, configures Cortes
...
PyYAML must be installed in the user's Python (it ships with most Cortes
...
the Cortes `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
