...
Maps each role archetype to the Cortes skills it should `always_load` and the
...
## Cortes skills relevant to video production
...
These are **not** Cortes skills but external CLIs / APIs that profiles invoke.
...
## Built-in Cortes tools for media review

These are native Cortes tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every Cortes profile (Claude) |
...
When a new Cortes-public video skill ships:
