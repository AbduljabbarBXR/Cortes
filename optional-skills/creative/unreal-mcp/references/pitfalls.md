...
### 1. Start order: editor first, Cortes second

Cortes probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Cortes session.
...
restart the editor, restart the Cortes session.
...
server fails to bind (Output Log) or Cortes' probe times out. Fix: change
...
### 5. GenerateClientConfig is not for Cortes

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Cortes' connection
lives in `~/.hermes/config.yaml` via `hermes mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Cortes — just don't
mistake it for the Cortes setup step.
...
explicitly warns against overlapping calls. Cortes executes same-turn tool
...
### 9. Timeouts: Cortes gives up before Unreal does

Cortes' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Cortes runs elsewhere (SSH backend,
