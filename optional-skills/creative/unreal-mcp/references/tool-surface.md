...
Under Cortes that means each tool becomes `mcp_unreal_engine_<tool_name>` at
...
| Server Port Number | `8000` | Change on conflict; mirror in Cortes config url |
...
| `ModelContextProtocol.GenerateClientConfig <Client\|All>` | Write client config files (ClaudeCode/Cursor/VSCode/Gemini/Codex) — NOT used for Cortes |
...
- **After Live Coding / authoring:** connected clients can hold stale
  schemas. `ModelContextProtocol.RefreshTools`, then reconnect (new Cortes
...
then re-`list_toolsets` from Cortes. Users on Claude Code can scaffold with
