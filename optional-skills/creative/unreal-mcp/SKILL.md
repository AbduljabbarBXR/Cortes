...
Companion skill for the `unreal-engine` entry in the Cortes MCP catalog. The
...
Two halves, in this order: the editor side must be up before Cortes connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Cortes — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Cortes connects
...
2. Start the Cortes session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Cortes
they appear as:
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Cortes runs batched calls
...
- **Start order matters.** Editor + server up first, then the Cortes
...
- **Timeouts on long operations.** Cortes' per-call default is 120 s; asset
