...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Cortes-Session-Id`` API header) and are otherwise interpolated raw
...
class AIAgent:
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Cortes against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Cortes can't reach the model provider. You may be offline. "
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Cortes-Session-Id header) and must not escape
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Cortes' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Cortes model request this cancels only that request;
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Cortes process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Cortes process was writing to the state "
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Cortes YOLO selected unrestricted
    ...
    def run_conversation(
        ...
        try:
            # Serialize the full load -> run -> flush region across Cortes
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Cortes process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Cortes process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Cortes process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Cortes process kept this session busy too "
