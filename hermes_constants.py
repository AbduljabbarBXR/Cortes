"""Shared constants for Cortes Agent.
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Cortes home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Cortes home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Cortes home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Cortes home path."""
...
def get_hermes_home() -> Path:
    """Return the Cortes home directory (default: platform-native path).
...
def hermes_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Cortes home/profile directory.
...
def get_process_hermes_home() -> Path:
    """Return the Cortes home for the running process, ignoring task overrides.
...
def get_default_hermes_root() -> Path:
    """Return the root Cortes directory for profile-level operations.

    In standard deployments this is the platform-native Cortes home
...
def get_hermes_dir(
    ...
    *,
    ...
    """Resolve a Cortes subdirectory with backward compatibility.
    ...
    Args:
        ...
        home: Optional explicit Cortes home. Profile-aware callers that manage
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Cortes-managed Node.js directories in preferred lookup order.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Cortes-managed Node trees live under ``$HERMES_HOME/node`` (or a profile's
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Cortes-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Cortes-managed Node.js is in use by a running app; deferring its "
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a Cortes-managed Node tree and return its npm path.
    ...
    Cortes never modifies a toolchain it does not own, so instead it provisions
...
def heal_hermes_managed_node() -> bool:
    """Redownload Cortes-managed Node when the tree exists but is broken.
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Cortes-managed Node/npm executable path, healing broken trees.
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Cortes-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Cortes-managed installs.

    This is for Cortes-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Cortes-managed Node directories prepended to PATH."""
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Cortes profile HOME.

    ``HERMES_HOME`` scopes Cortes state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Cortes' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Cortes itself runs inside WSL.
...
# Top-level packages/modules that ship as part of Cortes itself. An ImportError
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Cortes.
