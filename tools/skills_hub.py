#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Cortes Skills Hub.
...
# ---------------------------------------------------------------------------
# Cortes centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Cortes index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Cortes index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Cortes index fetch failed: %s", e)
            return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Cortes Skills Index.
