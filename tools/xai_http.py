...
def hermes_xai_user_agent() -> str:
    """Return a stable Cortes-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Cortes-Agent/{__version__}"
...
def hermes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Cortes Agent,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Cortes config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Cortes home, then mark it seen."""
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Cortes-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.hermes/.env`` (the standard Cortes location) are honored —
