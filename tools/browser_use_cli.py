...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Cortes's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Cortes's venv site-packages, and a
...
def _managed_bin_dir() -> Optional[str]:
    """Cortes' own bin dir ($HERMES_HOME/bin) — where install.sh puts uv/uvx
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Cortes' managed uv (bootstrapped on demand via
    ...
    # prevent provisioning the canonical Cortes-managed copy, or resolution
