...
# Cortes session id (observability identity, distinct from the gateway
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Cortes session id so observer plugins parent approval
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Cortes: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Cortes
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Cortes secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Cortes file.
    ...
    # In-place edit of a Cortes-managed security file (~/.hermes/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Cortes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Cortes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Cortes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    #
    # Fold the (more specific) Cortes home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hermes), so folding the user home
    # first would eat the prefix the Cortes-home fold needs.
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Cortes home prefix to ``~/.hermes/``.
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Cortes ad-hoc temp script."""
    try:
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Cortes YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Cortes approval prompts.
