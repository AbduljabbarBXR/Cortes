    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Cortes Agent"
"""
...
def _cached_redirect_port(storage: "HermesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Cortes restarts and chooses a
...
class HermesTokenStorage:
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Cortes records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Cortes.</p></body></html>"
            ) if code else (
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Cortes Agent" / "Cortes" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Cortes Agent")
    scope = cfg.get("scope")
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Cortes auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Cortes is running.
    """
    msg = str(exc)
    lowered = msg.lower()
    ...
    if _is_figma_remote_mcp(server_name, server_url):
        return (
            ...
            "work; most other names 403). Cortes defaults to "
