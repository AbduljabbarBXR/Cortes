...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Cortes-managed provider credential (API key,
    ...
    Non-Cortes API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Cortes credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Cortes-internal secrets (AUXILIARY_*_API_KEY /
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Cortes-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Cortes-managed
    ...
    Non-Cortes third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Cortes provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Cortes-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Cortes "
