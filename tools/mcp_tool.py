...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Cortes' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Cortes explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Cortes and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Cortes Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Cortes MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Cortes gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Cortes document cache; returns a marker with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Cortes profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Cortes' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Cortes' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this Cortes process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Cortes' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Cortes is fully
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Cortes processes (e.g.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Cortes config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Cortes' MEDIA: tag convention and out to messaging
            ...
            # Cortes' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Cortes's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Cortes uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Cortes registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Does NOT register tools in the Cortes registry.
