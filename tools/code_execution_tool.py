...
Lets the LLM write a Python script that calls Cortes tools via RPC,
...
# available on every platform Cortes itself runs on.
...
# operational marker: without it, a sandbox script that spawns/imports Cortes
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Cortes-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Cortes-profile soft guard."""',
...
"""Auto-generated Cortes tools RPC stubs."""
...
"""Auto-generated Cortes tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # HERMES_TIMEZONE is an internal Cortes setting and must not leak
        ...
        # Cortes's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Cortes's site-packages to that interpreter
        ...
        if _uses_hermes_python_environment(_child_python):
            ...
            logger.info(
                "execute_code: child interpreter %s is outside the Cortes "
...
# Interpreter paths already reported as outside the Cortes environment —
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Cortes's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that calls Cortes tools programmatically. "
