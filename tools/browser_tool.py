...
# means a compromised transitive dependency could read every Cortes secret
...
def _build_browser_env() -> dict:
    ...
    Strips Cortes-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Cortes profile."""
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Cortes silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Cortes-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Cortes CLI where sys.stdout and sys.stderr both report
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Cortes-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Cortes-managed one with no recovery — every candidate
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Cortes-managed
    ...
    # Build an extended search PATH including Cortes-managed Node, macOS
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    provider/gateway credential Cortes holds, while running registry-fetched
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Cortes
