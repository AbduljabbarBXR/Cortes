"""
Lazy dependency installer for opt-in Cortes Agent backends.

Many Cortes features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Cortes, which is what made it safe to seal
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Cortes under WSL to use Matrix on Windows."
        )
    return None
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv Cortes installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Cortes
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Cortes cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Cortes instead)."
            )
