...
def _build_child_agent(
    ...
    # child Cortes/Qwen subagent onto the parent's Claude Messages wire (or the
...
def delegate_task(
    ...
    # request-scoped chat_id binding (the raw X-Cortes-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Cortes-Session-Id once the batch completes.
...
def _load_config() -> dict:
    """Load delegation config from the active Cortes config.
