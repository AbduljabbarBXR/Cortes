...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Cortes convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Cortes infrastructure — it must NOT inherit
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Cortes Checkpoint"], store, cfg_wd)
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Cortes changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Cortes changed", not an exact per-session ledger.
