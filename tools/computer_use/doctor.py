...
flow through here without code changes on the Cortes side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Cortes provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Cortes-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    if json_output:
        ...
        # attach Cortes identity under hermes_identity so existing parsers
