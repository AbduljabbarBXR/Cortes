...
def _cua_no_overlay() -> bool:
    """True when Cortes should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Cortes should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Cortes runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Cortes process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Cortes session's YOLO choice
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Cortes an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    Cortes Desktop/TUI session can otherwise filter out the `computer_use`
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Cortes process is restarted.
            os.path.join(
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Cortes-managed secrets out of the child env.
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per Cortes run, with
        ...
        #     concurrent Cortes runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Cortes run = one backend = one session — and pass it as
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Cortes' minimal core.
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Cortes adapter, and uses cua-driver's
