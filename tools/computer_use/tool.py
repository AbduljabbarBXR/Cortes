...
# regardless of approval level (e.g. logout kills the session Cortes runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Cortes-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Cortes's explicit approval bypass onto Cua's immutable mode.

    Cortes has TWO session-identity namespaces: the tool-dispatch path passes
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Cortes YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Cortes sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Cortes process that spawned it
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Cortes sessions.
...
# "foreground" (it doesn't know Cortes has a typed page route), so the model
