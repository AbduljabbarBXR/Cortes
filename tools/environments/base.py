"""Base class for all Cortes execution environment backends.
...
# set), not Cortes' per-turn session identity.
#
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Cortes backends.
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # under Cortes via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Cortes-specific
        ...
        # inherited from the Cortes process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Cortes running inside another agent's terminal).
        parts.append(
        ...
        # Restrict Cortes metadata files without changing the user's command
