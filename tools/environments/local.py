...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Cortes was already launched from (for example ``hermes-agent``
    ...
    # Common recovery for config values like ``hermes-agent`` when Cortes was
...
# Cortes-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Cortes-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Cortes inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Cortes-managed inference credential — Claude subscription auth is not a
    # working Cortes provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Cortes venv path instead of the project's own
# ``.venv`` — silently clobbering the Cortes environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Cortes venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for Cortes-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Cortes-internal dynamic secret"
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Cortes home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Cortes-managed secrets from a subprocess environment."""
    try:
...
# legitimate child Cortes spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Cortes spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Cortes-internal dynamic secrets
...
def build_subprocess_env(
    ...
    *,
    ...
    Cortes secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Cortes home override into ``HERMES_HOME`` and apply the
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Cortes Agent requires Git for Windows on Windows.\n"
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Cortes. If the override is blocked or later re-applied, "
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Cortes-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Cortes installs for
    itself, so on a machine where Cortes provisioned its own toolchain a
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Cortes-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Cortes
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Cortes trusts them.
    """
