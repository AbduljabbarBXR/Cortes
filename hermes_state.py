#!/usr/bin/env python3
"""
SQLite State Store for Cortes Agent.
...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Cortes root for the guard.
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Cortes home *root*.
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Cortes root {root}). "
...
def _wal_reset_repair_hint() -> str:
    ...
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Cortes-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Cortes"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Cortes-managed install (uv-managed venv)
...
# threads inside ONE interpreter, yet a normal Cortes host runs several
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Cortes home tree (``get_hermes_home()``) — the safe repair scope:
      Cortes owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Cortes needs read-write access to open the database. "
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Cortes
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Cortes process held the "
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Cortes, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Cortes session.

        A Cortes session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Cortes session is already bound to any Telegram DM topic.
