...
## 2026-07-13: Scope plugin manager state by Cortes home/profile (keyed cache)

Status: Accepted

Context:
Cortes supports multiple profiles via different Cortes home directories.
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Cortes home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
