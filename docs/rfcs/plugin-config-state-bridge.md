...
or change arbitrary Cortes configuration.
...
small host fact requested by the RFC. Settings writes use Cortes'
...
- **Profile isolation:** the data root resolves from the active context-local
  Cortes home on every operation.
...
The implementation is covered with real temporary-Cortes-home tests for:
