...
has to be thrown away or summarized. Cortes has always done this in one batch:
...
Cortes folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Cortes's normal session flush is
...
With `micro_compact` unset or `false` Cortes behaves exactly as it always has:
