...
Cortes includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Cortes and does not require `hermes plugins enable
observability/nemo_relay`. Cortes remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Cortes execution remains available, while Relay scopes, middleware, plugins,
...
Cortes requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Cortes installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Cortes process so configured interceptors can
...
Collection remains off unless Cortes policy enables it:
...
Cortes core owns one Relay host and one isolated Relay session scope per Cortes
...
marks do not require Cortes plugin registration. Shared-metrics marks must
...
```text
Cortes turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Cortes shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Cortes sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Cortes used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; Cortes
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Cortes processes share the SQLite latch, so simultaneous
...
the owning Cortes session. The start counter contains only bounded execution
...
provider attempts for the same Cortes API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Cortes tool-call
...
cancellations. Active task ownership follows the task ID if Cortes rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. Cortes
...
tool or plugin names. Cortes does not infer retries from repeated tool names or
...
state, and a use-count bucket. Cortes derives reuse and patch-generation
...
are written with atomic replacement. Each package records the Cortes version,
...
The install identity is scoped to one `HERMES_HOME`. To reset it, stop Cortes
...
Run a real Cortes CLI turn against the deterministic local model server:
