...
Cortes gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Cortes-owned subscribers.
...
Cortes Agent-owned Relay transport health remains in scope. A separate gateway
...
release covers only Cortes Agent-owned service-health and operational-diagnostic
signals, including Cortes Agent-owned Relay transport health. Team Gateway's
