# Cortes Observer Hooks

Cortes observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Cortes' planner, model providers, memory, tool registry,
...
Cortes also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Cortes catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Cortes only builds
