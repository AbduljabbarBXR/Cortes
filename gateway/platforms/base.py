...
# Audio file extensions Cortes recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Cortes but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Cortes' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Cortes-created Telegram private-chat
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Cortes root.
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Cortes profile and shared Cortes root both contain control
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Cortes
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Cortes cache dirs.
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Cortes cache dirs (``/root/.hermes/...``), the default
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Cortes-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the Cortes root (~/.hermes/.env,
...
class BasePlatformAdapter(ABC):
    ...
    # Cortes commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Cortes sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Cortes home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Cortes home layout.
        """
