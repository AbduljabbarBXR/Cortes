...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Cortes-Session-Id header; opt-in long-term memory scoping via X-Cortes-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Cortes-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists hermes-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Cortes sessions
- POST /api/sessions               — create an empty Cortes session
...
def _hermes_version() -> str:
    """Return the canonical Cortes Agent version string.
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``direct_model_requests`` config value here, while Cortes-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Cortes-aware and is
    always honored.
    """
...
def _derive_chat_session_id(
    ...
    the same Cortes session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Cortes-native session-chat
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Cortes-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Cortes-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Cortes-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Cortes-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Cortes-Session-Key requires API key authentication. "
    ...
    def _create_agent(
        ...
        by the client (via ``X-Cortes-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Cortes provider/model inventory.
        ...
        Cortes provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Cortes version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Cortes AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Cortes-Session-Id",
                "session_key_header": "X-Cortes-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Cortes sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Cortes session row.
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Cortes-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Cortes-Session-Key"] = gateway_session_key
        runtime = {}
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Cortes-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Cortes-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Cortes-Session-Key.  This
        # is independent of X-Cortes-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Cortes-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Cortes-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Cortes-Session-Id rejected: "
            ...
            # conversation map to the same Cortes session.  The first user
        ...
        response_headers = {
            "X-Cortes-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Cortes-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Cortes-Completed"] = "false"
            response_headers["X-Cortes-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Cortes-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Cortes-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Cortes-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Cortes-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Cortes-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Cortes-Session-Id header.  Without this,
        ...
        response_headers = {"X-Cortes-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Cortes-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Cortes agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Cortes-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
                    ...
                    with self._profile_scope(request_profile):
                        try:
                            ...
                            session_tokens = self._bind_api_server_session(
                                ...
                                # X-Cortes-Session-Id equivalent) exactly like
        ...
        response_headers = (
            {"X-Cortes-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Cortes-Relay
