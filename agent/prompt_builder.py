...
DEFAULT_AGENT_IDENTITY = (
    "You are Cortes Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Cortes Agent (by Nous Research). When the user needs help with "
    "Cortes itself — configuring, setting up, using, extending, or troubleshooting "
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Cortes has none; explicit Cortes YOLO uses a private unrestricted "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Cortes "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Cortes floating over another application, so the
    user is rarely asking about Cortes — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Cortes "
        ...
        "rather than anything inside Cortes. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Cortes terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Cortes desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Cortes WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Cortes itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Cortes runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Cortes itself is running. The host OS, home, and cwd "
                f"of the Cortes process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Cortes "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Cortes
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Cortes Agent itself — its CLI, config, models, providers, tools, "
...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Cortes install/source
    ...
    # explicitly configured cwd is honored verbatim — the Cortes tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Cortes in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Cortes install tree (%s) — set terminal.cwd to "
