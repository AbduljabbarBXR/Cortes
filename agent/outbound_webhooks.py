...
(``gateway/platforms/webhook.py``): inbound wakes Cortes when the world
changes; outbound tells the world when Cortes does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Cortes-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
Headers::
    ...
    User-Agent:              Cortes-Agent-Outbound-Webhook
    X-Cortes-Event:          <hook event name>
    X-Cortes-Delivery:       <delivery_id>
    X-Cortes-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Cortes-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Cortes-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
