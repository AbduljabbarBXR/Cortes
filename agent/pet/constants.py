...
row taxonomy is inferred from the atlas shape so Cortes can render both legacy
...
# rows (1536x1872), while older Cortes/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Cortes' activity state names. They are not always identical to the
...
# Legacy Cortes/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Cortes activity names -> accepted row-name aliases in descending
