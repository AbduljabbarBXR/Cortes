...
* new gateway turns get a brief "Cortes is paused" reply instead of an
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Cortes is paused ({reason}). New work is on hold; "
            "run `hermes resume` to pick things back up."
        )
    return (
        "⏸️ Cortes is paused. New work is on hold; "
        "run `hermes resume` to pick things back up."
    )
