"""Upload a Cortes session transcript to Hugging Face as an agent trace.

Cortes stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Cortes' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Cortes OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Cortes' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Cortes conversation messages as Claude Code JSONL text.
