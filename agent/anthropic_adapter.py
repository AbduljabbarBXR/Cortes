"""Anthropic Messages API adapter for Cortes Agent.

Translates between Cortes's internal OpenAI-style message format and
...
# Cortes effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("X-Title", "Cortes Agent")
    ...
    # from ``ANTHROPIC_API_KEY`` (Cortes loads that into the process env from
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Cortes notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Cortes historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Cortes)
    ...
    # 1. Cortes-managed OAuth/setup token env var
    ...
    # 5. Cortes credential_pool OAuth entry.
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Cortes-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Cortes with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Cortes-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Cortes OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Cortes Agent", "Claude Code")
                text = text.replace("Cortes agent", "Claude Code")
        ...
        #      a) bare Cortes-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Cortes surfaces in its CLI. We
