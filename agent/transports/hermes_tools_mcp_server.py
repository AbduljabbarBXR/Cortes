"""Cortes-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
Cortes' richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those Cortes tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.hermes-tools]`) and
the user gets full Cortes capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - skill_view, skills_list              — Cortes' skill library
  - text_to_speech                       — TTS
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in Cortes
...
# Tools we expose. Each name MUST match a registered Cortes tool that
...
#     `_AGENT_LOOP_TOOLS` in Cortes (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. Cortes' default runtime
...
def _build_server() -> Any:
    """Create the FastMCP server with Cortes tools attached. Lazy imports
    ...
    # Discover Cortes tools so dispatch works.
    ...
    mcp = FastMCP(
        "hermes-tools",
        instructions=(
            "Cortes Agent's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative Cortes tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs Cortes gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this Cortes process", name
            )
            continue

        description = spec.get("description") or f"Cortes {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Quiet mode: keep Cortes' own banners off stdout (which is the MCP wire).
