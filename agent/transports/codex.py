...
# Wire-name used when Cortes keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Cortes's configured provider
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if params.get("is_xai_responses", False):
            ...
            # at high, while max/ultra remain Cortes aliases for every xAI model.
        ...
        if (params.get("provider") or "").strip().lower() == "actual":
            ...
            # line 1 column 1"). Clamp Cortes' wider set to the supported one.
            _effort_clamp.update({"xhigh": "high", "ultra": "max"})
        reasoning_effort = _effort_clamp.get(reasoning_effort, reasoning_effort)

        response_tools = _responses_tools(tools)

        # xAI server-side web search vs Cortes web providers.
        #
        ...
        #    keep Cortes dispatch so ``web.backend`` / ``web.search_backend``
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Cortes dispatches
