"""Projects codex app-server events into Cortes' messages list.

The translator that lets Cortes' memory/skill review keep working under the
...
Cortes' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Cortes
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Cortes' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Cortes stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Cortes already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Cortes' messages store text only.
        text_parts: list[str] = []
