...
Owns one Codex thread per Cortes session. Drives `turn/start`, consumes
...
# Cortes' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Cortes/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Cortes session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Cortes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Cortes' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Cortes' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Cortes
        approval-mode/timeout logic. The Cortes-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Cortes approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Cortes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Cortes returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Cortes approval mode/timeout resolution
    ...
    # "prompt expired"; the Cortes-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort Cortes version string for codex's userAgent line."""
    try:
