...
# un-throttled log fires on *every* selection. On Windows several Cortes
...
# readiness handshake ("Timed out connecting to Cortes backend after
...
class CredentialPool:
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Cortes process
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Cortes
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Cortes
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Cortes process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Cortes profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Cortes PKCE)
        ...
        # Cortes auth store or env vars, so resolve them here.
        ...
        # the Cortes auth store.  Without this gate the removal is instantly
        ...
        # Cortes owns its own Codex auth state — we do NOT auto-import from
...
# authoritative source for Cortes credentials. Stale env vars from parent
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # File-backed singletons (device-code OAuth, claude_code) and Cortes
