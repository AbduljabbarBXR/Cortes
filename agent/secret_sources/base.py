...
``~/.hermes/.env`` has loaded and BEFORE the rest of Cortes reads
``os.environ``.
...
# codes that must not reach Cortes' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Cortes knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Cortes
      output.
