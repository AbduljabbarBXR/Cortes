...
Cortes pulls API keys from Bitwarden Secrets Manager at process startup
...
* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  Cortes pins one version (``_BWS_VERSION``) and downloads
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block Cortes startup.  Missing binary, no network,
...
def _hermes_bin_dir() -> Path:
    """Where Cortes stores its managed binaries.  Profile-aware."""
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Cortes
