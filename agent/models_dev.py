...
# ---------------------------------------------------------------------------
# Provider ID mapping: Cortes ↔ models.dev
# ---------------------------------------------------------------------------

# Cortes provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Cortes' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Cortes ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Cortes provider ids that map to *mdev_id* (may be [])."""
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Cortes provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Cortes id.
...
def _get_provider_models(
    ...
    """Resolve a Cortes provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Cortes agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Cortes provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Cortes ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Cortes or models.dev provider ID.  Tries exact match then
