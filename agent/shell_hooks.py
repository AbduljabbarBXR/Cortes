...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Cortes-canonical
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Cortes wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Cortes
    ...
    if event == "pre_verify":
        # "continue" (Cortes) / "block" (Claude-Code Stop: block the stop) both
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Cortes is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical Cortes-wire-shape response — including exit-code-2
