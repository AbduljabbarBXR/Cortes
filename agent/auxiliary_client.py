...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Cortes owns auxiliary retry + provider/model fallback policy (the
    ...
    # Cortes sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Cortes control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Cortes triggers summarization.  Higher values delay
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Cortes Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Cortes plumbing (_reasoning_config
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Cortes auth store.
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Cortes' built-in discovery
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Cortes owns
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Cortes reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Cortes reasoning config for direct model calls
