...
Plugins built on Cortes Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Cortes
...
embedded slugs, no shorthands. This mirrors Cortes' main config
...
handles every provider, fallback chain, and per-task override Cortes
supports.
"""
