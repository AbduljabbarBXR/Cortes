...
## Layer 1 — Is this thing a Cortes plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Cortes process**. Its LLM calls go through Cortes's already-configured provider. | Already uses Portal if user's Cortes is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Cortes. That LLM call is already authenticated
via Cortes's provider config. The plugin's own server auth (e.g.
...
What Cortes Agent has that *feels* like OAuth — `hermes login --provider nous`
opening a browser, user signs in, token lands in `~/.hermes/auth.json` — is a
**Cortes-specific browser flow**. Under the hood it produces a credential
Cortes uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Cortes's existing Portal credential out of `~/.hermes/auth.json`.
...
rides on the credential Cortes already holds.

Where this could live in Cortes:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Cortes itself.
...
1. First decide: Cortes plugin (runs inside Cortes) or separate app? If plugin,
   it already uses Portal via Cortes's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Cortes agent itself") is fully supported. Cortes treats agent
...
pitch in the Tool Gateway announcement is specifically about Cortes Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Cortes is configured to use Portal as its provider. It is
