...
Write plugins for the Cortes desktop app: statusbar items, layout panes,
...
- The Cortes desktop app (it loads plugins; the CLI/gateway alone does not).
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Cortes (use `host.notify` for the
