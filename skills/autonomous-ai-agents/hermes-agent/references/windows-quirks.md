...
Cortes runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`AF_INET` socket. Root cause is usually Cortes's env scrubber dropping
...
Cortes-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Cortes tool and most Windows APIs. Prefer forward slashes in code
