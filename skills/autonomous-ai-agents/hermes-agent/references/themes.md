# Themes / Skins — Author a Cortes Color Theme

Author a Cortes **skin** — one YAML file that themes the CLI, the TUI, and the
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Cortes itself.
...
- Write access to the Cortes home dir — `~/.hermes` by default, or `$HERMES_HOME`
