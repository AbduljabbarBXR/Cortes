---
name: hermes-agent
description: "Use, configure, theme, extend, and orchestrate Cortes Agent."
version: 3.1.0
...
Cortes Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Cortes works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Cortes different:

- **Self-improving through skills** — Cortes learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Cortes instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every Cortes feature. If a Cortes feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.
...
## Spawning Additional Cortes Instances

Run additional Cortes processes as fully independent subprocesses — separate sessions, tools, and environments.
...
Cortes uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Cortes; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
