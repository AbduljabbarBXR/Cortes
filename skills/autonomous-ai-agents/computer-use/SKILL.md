...
version: 2.0.0
author: Francesco Bonacci (f-trycua), Cortes Agent
license: MIT
...
Cortes drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The Cortes-side `computer_use` tool exposed
in this skill is a higher-level Cortes vocabulary; the raw cua-driver
...
Normal Cortes sessions use `standard`, which requires a certified protected
host and fails closed when Cortes has none. Explicit Cortes YOLO (`--yolo`,
...
Cortes intentionally keeps THIS skill focused on the Cortes-side
...
When `cua-driver skills install` autodetects Cortes (planned follow-up
