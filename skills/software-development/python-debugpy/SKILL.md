...
For long-lived processes: Cortes gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Cortes you have two practical options:
...
## Debugging Cortes-specific Processes
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Cortes subagents, debug one process at a time.
