...
# Authoring Cortes-Agent Skills (in-repo)
...
author: Real Name (github-handle), Cortes Agent
license: MIT
...
- Credit the **human first**, then "Cortes Agent" as secondary collaborator: `Ben Barclay (benbarclay), Cortes Agent`.
- Never `author: Cortes Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Cortes Agent`.
...
|---|---|
| Cortes tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
3. **`author: Cortes Agent` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Cortes tools
