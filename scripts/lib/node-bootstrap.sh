...
#   2. ~/.hermes/node/ from a prior Cortes-managed install
...
# Redirect a Cortes-managed Node's `npm install -g` to the command link dir
...
# ---------------------------------------------------------------------------
# Heal a broken Cortes-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
heal_managed_node() {
    ...
    _nb_log "Cortes-managed Node is broken — redownloading to $HERMES_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        export PATH="$HERMES_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Cortes-managed)"
            HERMES_NODE_AVAILABLE=true
