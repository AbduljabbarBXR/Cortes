...
# Python-driven tool session (e.g. Cortes terminal tool). A pre-set PYTHONPATH
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Cortes.app)"
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Cortes installs the code under"
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Cortes-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Cortes Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Point a Cortes-managed Node's `npm install -g` at a directory that is on
...
# Cortes's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Cortes-managed npm, so calling it on
...
install_uv() {
    ...
    # Cortes owns its own uv at $HERMES_HOME/bin/uv.  Always install there —
...
# with the Cortes-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Cortes-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Cortes-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Cortes-managed Node from a previous run over a too-old system one.
    if [ -x "$HERMES_HOME/node/bin/node" ] && [ -x "$HERMES_HOME/node/bin/npm" ] \
        ...
        log_success "Node.js $("$HERMES_HOME/node/bin/node" --version) found (Cortes-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Cortes-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Cortes requires Node >=26) — installing Cortes-managed Node $NODE_VERSION..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Cortes install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Cortes Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Cortes behaves unexpectedly."
                    else
...
setup_path() {
    ...
    if [ ! -x "$HERMES_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$HERMES_ENTRYPOINT" ]; }; then
        log_warn "Cortes launcher prerequisites not found"
...
EOF
    ...
    # this launcher those hosts report Cortes as not installed. (#21454 applies
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Cortes Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Cortes Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Cortes Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Cortes Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Cortes will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Cortes Agent browser tools — explicit browser override."
...
install_browser_use_cli() {
    ...
    # MANAGED-FIRST: only Cortes' managed copy short-circuits. A browser-use
    ...
    # UV_TOOL_BIN_DIR keeps the binary inside Cortes' managed bin dir, where
...
install_computer_use_driver() {
    ...
    # so a stalled GitHub download can't hang the Cortes install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Cortes'
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Cortes to send/receive messages."
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.hermes-bootstrap-complete, which tells the Cortes
...
# extract a tree MISSING the electron binary, so the `electron`->`Cortes` rename
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Cortes-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Cortes" ]; then
            app="$desktop_dir/release/linux-unpacked/Cortes"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Cortes.app" \
            "$desktop_dir/release/mac/Cortes.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Cortes-managed Node
