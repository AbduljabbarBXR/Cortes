...
case "$MODE" in
  ...
  gate)
    ...
    expect "appimage (not under unpacked)"      skew     "$(decide --relaunch-target /opt/Cortes/hermes)"
  ...
  launch)
    ...
    if [ "$(uname)" != "Darwin" ]; then
      ...
      expect_msg "instant-exit relaunch downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Cortes' in d['message']"
    else
      ...
      expect_msg "missing bundle downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Cortes' in d['message']"
    fi
    ...
    stub_install
    bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/hermes-agent" \
      --relaunch-target /opt/Cortes/hermes >/dev/null 2>&1 || true
    if [ "$(uname)" != "Darwin" ]; then
