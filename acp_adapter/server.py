"""ACP agent server — exposes Cortes Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Cortes running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Cortes cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Cortes/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Cortes AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Cortes version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Cortes maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Cortes inventory is also used by ``hermes model``, the TUI,
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Cortes estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Cortes changes it.

        When the internal Cortes head rotated (e.g. compression-driven session
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Cortes' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Cortes setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Cortes
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Cortes on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Cortes' local kawaii waiting/status
        ...
        def _run_agent() -> dict:
            ...
            try:
                ...
                # Without it the prompt advertises the global Cortes workspace
        ...
        try:
            # Snapshot the internal Cortes DB session id before the turn so we
        ...
        # Cortes' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Cortes install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Cortes Agent v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Cortes has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
