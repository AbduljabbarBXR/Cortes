"""ACP session manager — maps ACP sessions to Cortes AIAgent instances.
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Cortes itself is running in WSL.
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Cortes from a Windows workspace while the ACP process runs
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Cortes agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Cortes AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Cortes runtime provider configuration.
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Cortes daemon's process cwd.
