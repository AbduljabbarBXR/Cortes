"""ACP auth helpers — detect and advertise Cortes authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Cortes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Cortes can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Cortes.
    ...
    not have Cortes provider credentials configured yet, so Cortes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Cortes using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Cortes provider",
            description=(
                "Open Cortes' interactive model/provider setup in a terminal. "
                "Use this when Cortes has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
