"""
Unified tool configuration for Cortes Agent.
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Cortes routes X searches through xAI's built-in x_search "
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Cortes telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # misses the uv Cortes installed and prefers a system one when both exist.
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Cortes process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Cortes-managed ``$HERMES_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Cortes installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Cortes-managed install)
        ...
        # (PATH -> Homebrew/Cortes-managed node -> npx) rather than a bare
        # shutil.which — Cortes-managed-Node-only setups resolve agent-browser
        ...
        # Cortes-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Cortes-managed-Node-only setups resolve npx only through the
        ...
        _print_info("    Restart Cortes for tracing to take effect.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Cortes Tool Configuration", Colors.CYAN, Colors.BOLD))
