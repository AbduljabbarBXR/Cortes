"""Cross-process mutual exclusion for in-flight Cortes updates.
...
against itself on every attempt ("Cortes is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ancestry check those users get exit 2 ("Cortes is still running") on every
  GUI update forever, with no Cortes process actually running.
"""
...
# to show "Cortes is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Cortes home (never the context-local profile override):
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Cortes update is already running (PID {holder.pid}, "
