...
def resolve_exec_command() -> str:
    ...
    Prefer the real ``hermes`` executable (argv[0] or PATH). When Cortes
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Cortes\n"
        "GenericName=Cortes Desktop\n"
        "Comment=Launch Cortes Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Cortes\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Cortes desktop entry. Return its path.
