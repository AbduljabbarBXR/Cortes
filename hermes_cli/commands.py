"""Slash command definitions and autocomplete for the Cortes CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Cortes config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Cortes works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Cortes' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Cortes is working", "Configuration",
    ...
    CommandDef("update", "Update Cortes Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Cortes Agent version", "Info", aliases=("v",),
...
# Telegram allows up to 100 BotCommands. Cortes ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Cortes
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to Cortes or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
