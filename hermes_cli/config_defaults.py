"""Default configuration data for Cortes Agent.
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    # SQLite journal mode used by every Cortes database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Cortes server processes.
    ...
    "agent": {
        ...
        # the Cortes-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Cortes
        ...
        # app, ACP) in a code workspace, Cortes adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Cortes auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Cortes sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Cortes
        ...
        # bundled Cortes image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Cortes sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Cortes should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before Cortes applies
    ...
    # single read_file page exceeds these limits, Cortes truncates the
    ...
    "compression": {
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Cortes' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Cortes does.
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Cortes appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # "Hey Cortes" hands-free wake word. Always-on, on-device hotword
    ...
    # response. If not, Cortes feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Cortes auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Cortes Agent*" header.
    ...
    # replace Cortes' built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        "acked_advisories": [],
        # Allow Cortes to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core Cortes tools (terminal, read_file, write_file, patch,
    ...
    # #8731).
    #
    # Provider keys accept the Cortes provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # behavior: files must live under the Cortes cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Cortes
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # Bitwarden due to NETWORK/TIMEOUT, Cortes may use this encrypted
    ...
    "computer_use": {
        ...
        # by default upstream. Cortes disables it for our users unless they
        # explicitly opt in here. When false (default), Cortes sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Cortes also calls
    ...
    # Cortes works exactly as before with `enabled: false`.
    "proxy": {
    ...
    # Cortes Desktop (Electron app) launch options. These only affect
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Cortes mints short-lived tokens from. Falls back to "
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Cortes auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Cortes' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Cortes, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Cortes API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Cortes API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Cortes API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
