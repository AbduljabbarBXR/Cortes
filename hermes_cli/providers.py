"""
Single source of truth for provider identity in Cortes Agent.
...
2. **Cortes overlays** — transport type, auth patterns, aggregator flags,
...
# -- Cortes overlay ----------------------------------------------------------
# Cortes-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Cortes-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Cortes overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Cortes overlay
    ...
    if overlay is not None:
        # Cortes-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Cortes overlay still advertises openai_chat
...
def resolve_provider_full(
    ...
    # 0.5 Exact Cortes provider IDs must win over LOSSY alias collapsing.
