"""Cortes update pipeline — extracted from ``hermes_cli/main.py``.
...
# Critical files that Cortes must be able to import immediately after an
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    """Update Cortes Agent by downloading a ZIP archive.
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Cortes behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Cortes couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Cortes couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Cortes behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Cortes couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Cortes couldn't drop "
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Cortes repository.")
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Cortes Desktop, exit any open `hermes` REPLs, and")
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        ...
        logger.debug("Could not snapshot active Cortes Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    try:
        ...
        logger.debug("Cortes Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Cortes Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Cortes' editable-install extras or
...
def _update_node_dependencies() -> list[str]:
    ...
    # Cortes profile using this checkout. Keep one per-checkout cache under the
    # shared Cortes root rather than rerunning npm once per named profile.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Cortes Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    ...
    PATH, so those hosts report Cortes as not installed even when it is.
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Cortes Agent — ACP launcher (written by `hermes update`).\n"
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Cortes processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Cortes Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Cortes desktop app / other Cortes terminals, then re-run:"
    )
...
def _orphaned_desktop_backend_pids(
    ...
    the update with "Cortes is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Cortes backend (``hermes_cli.main`` + ``serve`` /
      ``dashboard``), and
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Cortes...")
    try:
...
def _rebuild_desktop_after_update(
    ...
    #
    # Start the build subprocess with the Cortes-managed Node on PATH: when
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # re-check instead of dead-ending with "Cortes is still
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Cortes windows/gateways and re-run: hermes update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Cortes gateways, Desktop backends, or other "
        ...
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Cortes on PATH without
        ...
        # Warn if legacy Cortes gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Cortes gateway unit(s) detected:")
