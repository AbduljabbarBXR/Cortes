"""
Cortes Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Cortes.app`` or ``~/Applications/Cortes.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Cortes`` (NSIS per-user)
...
the app name ("Cortes"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Cortes``
  - Windows: ``%APPDATA%\\Cortes``
  - Linux:   ``$XDG_CONFIG_HOME/Cortes`` (default ``~/.config/Cortes``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Cortes"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Cortes"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Cortes" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Cortes.app"),
            home / "Applications" / "Cortes.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Cortes).
            local_base / "Programs" / "Cortes",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Cortes")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Cortes.desktop",
        ]
    return paths
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Cortes disappears from the launcher.
        try:
