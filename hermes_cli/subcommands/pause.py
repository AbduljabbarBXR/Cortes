...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Cortes paused"
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Cortes resumed — dispatch picks up on the next tick.")
    else:
        print(f"Cortes is not paused (no sentinel at {sentinel_path()}).")
    return 0
