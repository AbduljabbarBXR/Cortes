...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → Cortes + Honcho
    hermes version             Show version
    hermes update              Update to latest version
    hermes uninstall           Uninstall Cortes Agent
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Cortes' own profile selector.
        """
        try:
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Cortes itself has been explicitly configured (model
    ...
    # Only count these if Cortes has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Cortes to use their tokens.
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Cortes checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `hermes update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Cortes checkout, run `git restore -- ui-tui`\n"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        # and exit on an install whose only Node is the one Cortes installed,
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Cortes-managed
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Cortes — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Cortes still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Cortes isn't configured yet -- no API keys or providers found."
        )
        print()
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Cortes?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Cortes Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
#
# Cortes uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Cortes only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Cortes URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Cortes will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Cortes CLI with a provider."""
...
def cmd_backup(args):
    """Back up Cortes home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Cortes backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Cortes Agent (or just the Chat GUI with --gui)."""
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Cortes-managed install, or a freshly provisioned managed npm when
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Cortes.app/Contents/MacOS/Cortes → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Cortes.app/Contents/MacOS/Cortes"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Cortes.exe",
            release_dir / "win-ia32-unpacked" / "Cortes.exe",
            release_dir / "win-arm64-unpacked" / "Cortes.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Cortes",
            release_dir / "linux-arm64-unpacked" / "hermes",
            release_dir / "linux-arm64-unpacked" / "Cortes",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Cortes.exe
...
# Cortes.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Cortes.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Cortes.exe``.
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Cortes.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Cortes.exe from backup.")
    ...
    print("    Run `hermes desktop --force-build` to rebuild, or re-run the Cortes")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Cortes'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Cortes.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Cortes.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Cortes.exe: Access is
    ...
    an unrelated "Cortes" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Cortes.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Cortes is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Cortes.app/Contents/MacOS/Cortes  ->  app bundle = .../Cortes.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Cortes Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Cortes Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Cortes Desktop (Linux only, best-effort).
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Cortes-managed Node on PATH so npm's child scripts (which
            ...
            if not source_mode:
                ...
                # holds Cortes.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Cortes.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Cortes.exe, close any")
                    print("  running Cortes desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Cortes is
                ...
                # success on a Cortes.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Cortes shows up
    ...
    if source_mode:
        print("→ Launching Cortes Desktop from source build...")
    ...
    print(f"→ Launching packaged Cortes Desktop: {' '.join(launch_command)}")
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Cortes installs Linux services in the
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        ...
        if self_locked:
            print(
                "  Cortes is still running from the launcher that needs "
                "replacing. Close other Cortes windows, restart from a "
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Cortes Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Cortes Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Cortes Desktop, exit other `hermes` REPLs, stop the "
...
def cmd_update(args):
    """Update Cortes Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Cortes Agent")
        return
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Cortes {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Cortes {mf.hermes_requires}")
...
def cmd_dashboard(args):
    ...
    # the same logs directory as every other Cortes surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def cmd_logs(args):
    """View and filter Cortes log files."""
...
def cmd_console(args):
    """Open the safe Cortes command console."""
...
def cmd_acp(args):
    """Launch Cortes Agent as an ACP server."""
    try:
...
def _advertise_agent_env() -> None:
    ...
    ``HERMES_AGENT`` is the Cortes-specific marker. setdefault: never
    clobber an outer harness (e.g. Cortes running inside another agent's
    terminal).
    """
...
def main():
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Cortes shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Cortes app. `status` reports the driver's grant\n"
    ...
    # for one release. The Cortes-Setup.exe success screen tells users to
