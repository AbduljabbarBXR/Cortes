...
def _scan_dashboard_processes(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Cortes Desktop Electron app to protect its own
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Cortes Desktop (or become PPID-1
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    # When the Cortes Desktop Electron app spawns this dashboard as a
...
def _detect_concurrent_hermes_instances(
    ...
    The Cortes Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    #      hermes.exe sitting *under* a non-Cortes parent (e.g. a Cortes
...
def _hermes_home_dir() -> Path:
    """Resolved Cortes home (HERMES_HOME override or ~/.hermes)."""
