"""
Interactive setup wizard for Cortes Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Cortes Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Cortes using environment variables or config commands:")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Cortes cannot chat yet.")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Cortes auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Cortes runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        try:
            ...
            # which() misses the uv Cortes installed. Bootstrapping one is
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Cortes delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Cortes to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Cortes from anywhere.")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Cortes profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and Cortes.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Cortes to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Cortes at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Cortes at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Cortes at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Cortes at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Cortes equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Cortes config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Cortes.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Cortes's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Cortes can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Cortes configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Cortes config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Cortes (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Cortes session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⚕ Cortes Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Cortes Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Cortes Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Cortes Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Cortes configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Cortes?",
            [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Cortes to messaging apps to chat from anywhere.")
