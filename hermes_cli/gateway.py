...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Cortes profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    ...
    manager. Starting Cortes's detached watcher as well would escape the
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Cortes Docker image — that one runs s6-overlay as
...
SERVICE_DESCRIPTION = "Cortes Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Cortes root to compare against. Used when
...
# Legacy service names from older Cortes installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Cortes gateway units.

    Detects unit files installed by older Cortes versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Cortes gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Cortes gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Cortes gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Cortes gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Cortes gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Cortes gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Cortes often sets ``HOME`` to a profile-scoped directory, but
...
def _append_node_dir_for_service(
    ...
    The Cortes-managed Node under ``$HERMES_HOME/node`` goes first when it
    ...
    *hermes_root* is the Cortes home the unit will run against. System units
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Cortes home is known — probing here would stat the calling
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Cortes install"
    )
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Cortes install"
    )
    return True
...
def launchd_status(deep: bool = False):
    ...
    # Cortes PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Cortes PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Cortes install")
    else:
        print("⚠ Service definition is stale relative to the current Cortes install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Cortes gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Cortes command."
    )
    print(
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Cortes delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Cortes connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Cortes will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Cortes will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Cortes will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # Stop, disable, and remove legacy Cortes gateway unit files from
