"""
Configuration management for Cortes Agent.
...
#   the next ``subprocess.run([...])`` Cortes makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Cortes itself starts
...
#   ``HERMES_ENV`` — Cortes runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Cortes runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Cortes runtime location "
...
def is_managed() -> bool:
    """Check if Cortes is running in package-manager-managed mode.
...
_NIX_UPDATE_MSG = (
    "Update Cortes through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Cortes was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
...
#     — that script installs a *new* host-side Cortes, it doesn't update
...
Cortes Agent runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this Cortes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Cortes installation is managed by NixOS "
    ...
    return (
        f"Cannot {action}: this Cortes installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Cortes."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Cortes commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When Cortes runs in a container with volume-mounted config files, forcing
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Cortes' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Cortes versions) may write
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Cortes won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Cortes host rewrites it to the host or container
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Cortes writes itself."""
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Cortes — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Cortes vars that are no longer in .env
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for Cortes-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Cortes Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
