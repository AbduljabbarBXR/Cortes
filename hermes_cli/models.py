...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Cortes-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Cortes that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Cortes that doesn't ship
...
#
# Some vendors expose several Cortes provider slugs (one per endpoint /
...
# In-repo fallback for the model Cortes silently lands on when the user never
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Cortes lands on when none is selected.
            desc = "default"
        else:
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Cortes' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Cortes should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Cortes release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Cortes release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Cortes release.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Cortes' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Cortes' native Anthropic adapter. The live catalog may
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Cortes will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Cortes cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Cortes actually ships.  (#46850)
            #
