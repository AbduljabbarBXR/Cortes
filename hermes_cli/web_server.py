...
app = FastAPI(title="Cortes Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Cortes-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Cortes-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Cortes (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Cortes provider slug — fine.
    ...
    1. Vendor-name → Cortes-provider mapping: when the provider string is
       not a known Cortes provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Cortes
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Cortes credential-store basenames (see
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the hermes-webui image sharing the Cortes source tree),
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Cortes root is actually /opt/data
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Cortes inside an xterm.js PTY. Browser
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Cortes Cloud
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Cortes updates are managed outside this dashboard in "
...
async def check_hermes_update(force: bool = False):
    """Report whether a Cortes update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed Cortes version string
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Cortes supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Cortes from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Cortes to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Cortes from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Cortes to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Cortes in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Cortes through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Cortes via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Cortes through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Cortes to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Cortes inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Cortes to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Cortes through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Cortes to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Cortes to Microsoft Teams chats via the Bot Framework.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Cortes.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Cortes from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Cortes over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Cortes through iMessage via Photon's managed Spectrum platform.",
    ...
    "simplex": {
        "description": "Talk to Cortes over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Cortes to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Cortes as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "whatsapp_cloud": {
        ...
        "description": "Use Cortes via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Cortes Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Cortes Agent").strip() or "Cortes Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Cortes appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Cortes-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Cortes PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Cortes even
    when they also have a separate Cortes-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Cortes never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Cortes, so the disconnect
    ...
    user sees exactly what executes, and Cortes then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Cortes — run the disconnect command to remove it."
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Cortes-managed PKCE file and auth-store entry.
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Cortes file AND credential pool.
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Cortes "
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Cortes,
...
def _resolve_chat_argv(
    ...
    # Cortes TUI child: build via the single spawn-env factory (profile-home
...
# ---------------------------------------------------------------------------
# /api/console — safe Cortes Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Cortes CLI
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Cortes Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Cortes inside WSL2 to use the dashboard's /chat "
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Cortes Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Without this the SPA paints the default Cortes Teal canvas, then
    ...
    # (the MC Pages app), not the Cortes backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Cortes Teal",         "description": "Classic dark teal — the canonical Cortes look"},
    {"name": "default-large", "label": "Cortes Teal (Large)", "description": "Cortes Teal with bigger fonts and roomier spacing"},
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Cortes Agent exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Cortes backend listening on {host}:{actual_port}")
            else:
                print(f"  Cortes Web UI → http://{host}:{actual_port}")
