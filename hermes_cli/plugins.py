...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Cortes loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Cortes' shipped guidance lives in the evidence-based
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Cortes home."""
...
# Highest manifest schema version this Cortes understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Cortes "
...
class PluginManifest:
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Cortes never
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Cortes paths, traversal syntax, and the security-
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Cortes config writers still retain their existing
    ...
    def profile_name(self) -> str:
        """Return the active Cortes profile name (e.g. ``"default"``).
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Cortes core) are trusted by default —
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.hermes/.env`` loads, before Cortes reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Cortes' Slack adapter wires registered handlers into its
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # path actually asks for that platform. Every platform Cortes ships
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Cortes process (e.g. a pip
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Cortes validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Cortes does not install plugin dependencies "
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Cortes home (profile switch reusing a slug
...
# Keyed cache: resolved Cortes home -> PluginManager. Cortes supports
...
def _plugin_home_key() -> Path:
    ...
    can temporarily switch Cortes home (env var *or* the context-local
    ``set_hermes_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Cortes home instead of
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Cortes profile/home.
