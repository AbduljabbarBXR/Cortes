"""Direct NeMo Relay integration for Cortes shared client metrics."""
...
class _Runtime:
    """Own shared-metrics state layered on the Cortes core Relay host."""

    def __init__(self, host: relay_runtime.RelayRuntime | None = None) -> None:
        resolved_host = host or relay_runtime.get_runtime()
        if resolved_host is None:
            raise RuntimeError("Cortes core Relay runtime is unavailable")
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Cortes task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # physical attempt. Provider fallback resets Cortes's
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Cortes retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Cortes shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Cortes shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Cortes shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_tool_call(
        ...
        try:
            ...
            logger.warning(
                "Cortes shared-metrics tool call close failed",
                exc_info=True,
            )
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Cortes shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Cortes shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Cortes shared metrics operation failed", exc_info=True)
            return None


def enabled() -> bool:
    """Return the shared-metrics policy for the active Cortes profile."""
    ...
    try:
        ...
        logger.debug("Unable to read Cortes shared-metrics policy", exc_info=True)
        value = False
    else:
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Cortes lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning(
            "Cortes shared metrics hook failed: %s", hook_name, exc_info=True
        )


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Cortes's runtime registry."""
...
def start_task_run(
    *,
    ...
    """Start task metrics at the outer Cortes execution boundary."""
    if not enabled():
        return
    ...
    *,
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Cortes shared metrics initialization failed", exc_info=True)
