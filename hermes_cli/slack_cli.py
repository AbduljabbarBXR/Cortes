...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Cortes. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Cortes deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Cortes on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Cortes in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Cortes in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Cortes agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Cortes")
    ...
    """
    name = getattr(args, "name", None) or "Cortes"
    description = getattr(args, "description", None) or "Your Cortes agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Cortes app\n"
