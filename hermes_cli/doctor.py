...
Diagnoses issues with Cortes Agent setup.
"""
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Cortes containers"
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Cortes-managed SQLite databases."""
...
def _report_database_journal_modes(
    ...
    try:
        ...
        check_warn(f"Could not list Cortes databases: {exc}")
        return
    exposed = []
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Cortes Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Cortes already refuses to enable WAL on fresh DBs.
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Cortes' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Cortes a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Cortes Agent Persona\n\n"
                "<!-- Edit this file to customize how Cortes communicates. -->\n\n"
                "You are Cortes, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        ...
        # with a runaway WAL was previously invisible to every Cortes
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Cortes one.
                        check_info(
