"""On-demand supply-chain audit for Cortes Agent installs.

Scans three surfaces a Cortes user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Cortes venv (every PyPI dist via ``importlib.metadata``).
