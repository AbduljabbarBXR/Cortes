...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Cortes' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Cortes-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Cortes agent → needs provider credentials.
        ...
        # Prepend the Cortes venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Cortes-managed CLIs
...
def _ensure_skin_watcher() -> None:
    ...
    Cortes activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    the same backend told the model it was "chatting inside the Cortes desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base Cortes): with no explicit pin, collapse to the
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Cortes window, and answers
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Cortes has to support both the legacy 8-row petdex atlas and the current
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Cortes sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Cortes
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Cortes" if role == "assistant" else role.title()
